...
class L5PKeyboard:
    VENDOR_ID = 0x048D
    PRODUCT_IDS = [
        0xC995, 0xC994, 0xC993, # 2024
        0xC985, 0xC984, 0xC983, # 2023
        0xC975, 0xC973,         # 2022
        0xC965, 0xC963,         # 2021
        0xC955                  # 2020
    ]
    ...
    def _find_device(self):
        ...
        for dev_info in hid.enumerate(self.VENDOR_ID):
            if dev_info['product_id'] in self.PRODUCT_IDS:
                ...
                if dev_info['usage_page'] == self.USAGE_PAGE and dev_info['usage'] == self.USAGE:
    ...
    def set_effect(self, effect):
        if effect not in self.EFFECTS:
            raise ValueError(f"Invalid effect. Choose from: {list(self.EFFECTS.keys())}")
        self.effect = effect
        self.refresh()
