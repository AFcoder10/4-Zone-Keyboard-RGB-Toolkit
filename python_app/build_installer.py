...
class PythonInnoSetupGenerator:
    def __init__(self, app_name, app_version, app_publisher, app_exe, source_dir, output_dir):
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(SOURCE_DIST_DIR, APP_EXE)):
        ...
        print("You must run `build_exe.py` and `update_exe.py` normally first to create the standalone application bundle before it can be wrapped into an installer.")
        sys.exit(1)
    ...
    if success:
        print(f"Your final setup installer is waiting at: {os.path.join(FINAL_OUTPUT_DIR, '4_Zone_Rgb_Toolkit_Setup.exe')}")
